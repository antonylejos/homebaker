...
module "gke" {
  ...
  # Scheduled scale-down is disabled in dev
  enable_scheduled_scaledown = false
}
...
resource "google_monitoring_alert_policy" "baker_service_restart" {
  display_name = "homebaker baker-service CrashLoopBackOff"
  combiner     = "OR"

  conditions {
    display_name = "baker-service container restart rate"
